...
### 2026-09-23 — Python block indent (2.10 follow-up)
- **Shipped:** `auto_indent` now adds one level after a line ending in `:` when the buffer is Python (`.py`/`.pyi`), covering `def`/`if`/`for`/`while`/`class`/`with`/`try`/`else`/… — the bracket heuristic missed these since Python blocks aren't delimited by `{}`. Gated on the file extension (`buf_is_python`) so a trailing colon in other languages (e.g. a Rust label) is not affected. (Comment-after-colon and statement-based dedent — `return`/`pass` — remain part of the larger full-tree-sitter-indent follow-up.)
- **Tests:** 1 Rust unit (`def f():` gains a level, a non-colon line just copies indent, a `.rs` `label:` does not indent) + tests/pty_python_indent.py (2 geometries: typing `if x:`⏎`pass` yields `if x:\n    pass` on disk).
- **Verified:** 562 Rust tests pass; clippy clean; PTY green.
