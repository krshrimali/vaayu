...
- [~] 2.1 Semantic-token highlighting: `:set semantictokens` requests `textDocument/semanticTokens/full`, decodes the delta stream via the server legend, and overlays token colors (keyword/type/function/string/comment/number) on top of tree-sitter; **the `deprecated` modifier draws the token struck-through**. Full-buffer only; delta/range updates + more modifiers = follow-up — **M**
...
- [ ] conceal support — **M**
- [~] 2.11 Configurable statusline: `statusline` config format string (`%f`/`%F`/`%l`/`%c`/`%L`/`%m`/`%y`/`%p`/`%M`/`%%`), ruler stays on the right. **Winbar done** (`:set winbar`: per-pane top row with the relative path + tree-sitter enclosing-symbol breadcrumb, content shifts down, mouse-mapping aware). **Global statusline done** (`:set globalstatusline`/`laststatus3`: one shared status for the active window, per-pane statuses reclaimed as content); **multi-level winbar breadcrumb done** (full enclosing path, e.g. `impl Foo › fn bar`). statuscolumn = remaining — **M**
