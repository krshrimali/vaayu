...
- [~] 2.1 Semantic-token highlighting: `:set semantictokens` requests `textDocument/semanticTokens/full`, decodes the delta stream via the server legend, and overlays token colors (keyword/type/function/string/comment/number) on top of tree-sitter; **the `deprecated` modifier draws the token struck-through and `readonly` draws it italic**. **Incremental `full/delta` updates done** (after the first full request the server's `resultId` is kept and later edits send `semanticTokens/full/delta`; the returned edits are spliced into the raw token stream before decoding, when the server advertises `full.delta`). Viewport-`range` requests = optional micro-optimization remaining — **M**
...
- [~] 2.5 Document color: `,lC` (`lsp.document_color`) requests `textDocument/documentColor` and paints each color literal in its own RGB; clears on edit/Esc. **Background swatch chips done** (`:set colorswatch`: each color literal is drawn as a chip — its own RGB as the *background* with a luminance-contrasted fg — instead of only tinting the fg). **Color adjustment done** (`:colorpick` reports the `#rgb`/`#rrggbb` under the cursor; `:colorlighten [pct]`/`:colordarken [pct]` scale it toward white/black and rewrite as `#rrggbb`). A full interactive modal picker = follow-up — **S**
...
- [~] 2.7 Project-wide replace: `:cfar/pat/repl/[flags]` (also `:cfar /pat/repl/`) rewrites every file in the current results/quickfix list (e.g. from a prior `:grep`), reusing `run_substitute` so regex/flags/capture-group semantics match `:s`; saves each changed file and refocuses the original buffer. **Preview done** (`:cfarpreview /pat/repl/[flags]` lists every line the matching `:cfar` would change across the results files in a Results panel — reading open buffers live and the rest from disk — without modifying anything; run `:cfar` to apply). Per-hunk accept/reject UI = remaining — **M–L**
...
- [~] 6.x completeness: `:checkhealth` **done**; config surface: **`cursorline`, `colorcolumn`, `list`, + runtime `:set` for `relativenumber`/`ignorecase`/`smartcase`/`smartindent`/`expandtab`/`autopairs` and `tabstop`/`shiftwidth`/`scrolloff`/`textwidth`/`updatetime`=N done** (short forms too); **large-file mode done** (`large_file_kb`, default 5 MiB; `:set largefilekb=N`) — files over the cutoff skip tree-sitter/spell/TODO/rainbow scans; **multi-tab session save/restore done** (`:sessionsave`/`:sessionload` now round-trip all tabs + the active tab, not just tab 1); **configurable `listchars` done** (`listchars="tab:xy,trail:z"`); **EditorConfig done** (`end_of_line`, `charset`, `trim_trailing_whitespace`, `insert_final_newline`, `max_line_length` + indent keys, all per-file); **configurable `fillchars` done** (`eob`, `vert`); **session fold-state persistence done** (`:sessionsave` records each buffer's closed/open fold ranges keyed by path; `:sessionload` restores them onto the reopened buffers, clamped to the current line count — back-compatible via a `#[serde(default)]` field so older session files still load); **Ex command set extended: `:normal[!] {keys}`** (runs normal-mode keys via the replay path), **`:[range]delete`**, and **`:global`/`:g` + `:vglobal`/`:v`** (`:g/pat/cmd` runs `cmd` — e.g. `d`, `s///`, `normal …` — on each matching line, `:v` on non-matching; matched lines processed bottom-up for line-count stability); **`:[range]sort[!] [u][n][i]`** (alphabetical / reverse / unique / numeric-by-first-number / case-insensitive, whole buffer or a range); **`:[range]move {addr}` / `:[range]copy {addr}`** (`:m`/`:t`/`:co`, destination abutting the letter `:2m4` or spaced `:2m 4`, `0` = top); **`:[range]join[!]`** (`:j`, single-space join with leading-whitespace trim, `!` = verbatim concat); **`:[range]>`/`:<`** (repeatable `:>>`, shift by `shiftwidth`, reusing the normal-mode indent so blank lines stay untouched); **`:[range]yank [reg]` / `:[range]put [reg]`** (`:y`/`:pu`, linewise via the register store, put below the target line, no-trailing-newline safe at EOF); **`:[range]!cmd`** (filter the range through a shell command — stdin=lines, stdout replaces them, via a writer thread to avoid deadlock — or `:!cmd` with no range runs it and lists output); session completeness = remaining — **M**
...
### 2026-09-24 — :!cmd filter Ex command (6.x completeness)
- **Shipped:** `:[range]!cmd` filters the range lines through a shell command (their text on stdin, the command's stdout replacing them); `:!cmd` with no range runs the command and lists its output in a Results panel. stdin is written on a background thread so a large output can't deadlock the wait; the replaced span's trailing-newline shape is preserved. Uses `sh -c`, matching the existing user-invoked runner (`:make`); the command is the user's literal input (Vim `:!` semantics), so no shell string is built from untrusted data.
- **Tests:** 1 Rust unit (`:%!sort`, range `:1,2!tr a-z A-Z`, EOF no-trailing-newline preserved) + tests/pty_filter.py (2 geometries: `:%!sort` on `3/1/2` → `1/2/3` on disk).
- **Verified:** 578 Rust tests pass; clippy clean; PTY green.

### 2026-09-24 — :yank / :put Ex commands (6.x completeness)
- **Shipped:** `:[range]yank [reg]` (`:y`) stores the range (or current line) linewise into a register (unnamed by default, or a named `a`–`z`/digit); `:[range]put [reg]` (`:pu`) pastes that register's lines below the target line. Reuses the shared `Registers` store (so it interoperates with normal-mode `yy`/`p`), and handles putting at EOF when the file has no trailing newline (inserts a leading `\n` instead of merging).
- **Tests:** 1 Rust unit (`:1,2yank a` + `:4put a`; unnamed yank/put; put at a no-trailing-newline EOF) + tests/pty_yankput.py (2 geometries: `:1,2y` then `:$put` → `a\nb\nc\na\nb` on disk).
- **Verified:** 577 Rust tests pass; clippy clean; PTY green.

### 2026-09-24 — :>/:< line-shift Ex commands (6.x completeness)
- **Shipped:** `:[range]>` and `:[range]<` shift the range (or current line) by `shiftwidth`, repeatable as `:>>`/`:<<` (once per `>`/`<`), reusing `operator::indent_lines` so behavior matches normal-mode `>>`/`<<` (blank lines left untouched under `>`). A fallback dispatch arm matches a name made only of `>` or only of `<`.
- **Tests:** 1 Rust unit (`:1,2>` then `:1,2<` round-trip, `:>>` doubles, `:%>` skips a blank line) + tests/pty_shift.py (2 geometries, `shiftwidth=2`: `:1,3>` then `:1,3<` round-trip on disk).
- **Verified:** 576 Rust tests pass; clippy clean; PTY green.

### 2026-09-23 — :join (6.x completeness)
- **Shipped:** `:[range]join[!]` (`:j`). With no range (or a single line) it joins the current line with the next; a range collapses all its lines into one. Default joins with a single space after trimming each subsequent line's leading whitespace (no double space); `!` concatenates verbatim. One edit; a missing final newline is preserved when joining at EOF; the cursor lands at the join.
- **Tests:** 1 Rust unit (default join trims leading ws; range `:1,3join`; `:join!` verbatim; EOF join keeps no-trailing-newline) + tests/pty_join.py (2 geometries: `:1,3join` → `one two three\nkeep` on disk).
- **Verified:** 575 Rust tests pass; clippy clean; PTY green.

### 2026-09-23 — :move / :copy (6.x completeness)
- **Shipped:** `:[range]move {addr}` (`:m`) and `:[range]copy {addr}` (`:t`/`:co`) relocate/duplicate the range after destination line `{addr}` (`0` = before the first line). `parse_move_copy` accepts the address abutting the command letter (`:2m4`) or after a space (`:2m 4`), and only matches when an address-like char follows so `:messages`/`:tabnew`/`:colorscheme` aren't hijacked; the destination reuses `parse_one_address` (`.`/`$`/`'m`/N/±N). Moving a range into itself is refused. Rebuilt in one edit, trailing-newline preserved.
- **Tests:** 1 Rust unit (`:2m4`, spaced `:m $`, `:1t$`, `:m0` to top, range `:1,2t$`, into-itself refusal, `:messages` not hijacked) + tests/pty_movecopy.py (2 geometries: `:1m$` then `:1t$` on disk).
- **Verified:** 574 Rust tests pass; clippy clean; PTY green.

### 2026-09-23 — :sort (6.x completeness)
- **Shipped:** `:[range]sort[!] [u][n][i]` — alphabetical by default, `!` reverses, `u` drops adjacent duplicates, `n` sorts by each line's first integer (`leading_number`, missing → 0), `i` is case-insensitive; operates on a range or the whole buffer. One undo step.
- **Tests:** 1 Rust unit (plain, `!`, `n`, `u`, a line-range sort leaving header/footer, and `i`) + tests/pty_sort.py (2 geometries: `:sort` then `:sort!` round-trip on disk).
- **Verified:** 573 Rust tests pass; clippy clean; PTY green.

### 2026-09-23 — :normal, :global/:vglobal, :delete (6.x completeness)
- **Shipped:** three core Vim editing commands that were missing. `:normal[!] {keys}` feeds the literal keys as normal-mode input via the existing macro-replay path (so recursion/budget limits and the recording guard apply) and ends any pending Insert. `:[range]delete` (`:d`) removes the range or current line. `:global`/`:g` and `:vglobal`/`:v` (`<word><delim>pat<delim>cmd`) collect the matching (or non-matching) lines and run `cmd` on each — `parse_global` only matches when a non-alphanumeric delimiter follows, so `:gitdiff`/`:vsplit` are never hijacked; matched lines run bottom-up so `:g/pat/d` stays index-stable. Together these unlock `:g/pat/d`, `:g/pat/s///`, and `:g/pat/normal …`.
- **Tests:** 3 Rust unit (`:delete` current+range; `:normal` incl. `A…<Esc>`, empty no-op, `:normal! dd`; `:g/:v` with `d`/`normal`/`s`) + tests/pty_global.py (2 geometries: `:g/TODO/d` then `:g/keep/normal A!` → `keep a!\nkeep b!\nkeep c!` on disk).
- **Verified:** 572 Rust tests pass; clippy clean; PTY green.

### 2026-09-23 — color adjustment commands (2.5 follow-up)
- **Shipped:** `:colorpick` reports the hex color under the cursor (`#rgb`/`#rrggbb` → `rgb(r,g,b)`); `:colorlighten [pct]` and `:colordarken [pct]` (default 10%) scale each channel toward white/black and rewrite the token as `#rrggbb` (normalizing a 3-digit form). A non-modal color tool — a full interactive modal picker remains a follow-up. `color_at_cursor` scans the line for the `#…` token covering the cursor column.
- **Tests:** 1 Rust unit (lighten `#808080`→`#bfbfbf`, darken →`#606060`, 3-digit `#f00`→`#ff0000` at 0%, no-op without a hex) + tests/pty_colorpick.py (2 geometries: `f#` then `:colorlighten 50` writes `#bfbfbf` to disk).
- **Verified:** 569 Rust tests pass; clippy clean; PTY green.

### 2026-09-23 — merge-conflict resolution / 3-way merge (completes 4.1)
- **Shipped:** a new `src/conflict.rs` parses git conflict blocks (`<<<<<<<` … optional diff3 `|||||||` base … `=======` … `>>>>>>>`) and resolves the one under the cursor: `:conflictours`/`:conflicttheirs`/`:conflictboth` replace the whole block with the chosen side(s) and remove the markers (the base section is always discarded); `:conflictnext`/`:conflictprev` jump between blocks. Line-oriented, edit-based (one undo step), touching no core engine path; malformed/unterminated blocks are skipped. This was 4.1's last remaining piece.
- **Tests:** 3 Rust unit (keep ours/theirs/both on a 2-way conflict; diff3 base dropped + next/prev navigation across two blocks; resolve outside a block is a no-op) + tests/pty_conflict.py (2 geometries: `:conflictnext`→`:conflicttheirs` yields `top\ntheirs line\nbottom\n` on disk with the markers gone).
- **Verified:** 568 Rust tests pass; clippy clean; PTY green.

### 2026-09-23 — conceal support (completes the conceal item)
- **Shipped:** `:set conceal` + `[[conceal_rules]]` config (`{ pattern, cchar }`). Each rule's regex matches on a line are replaced by `cchar`'s first char (dim) — or hidden entirely when `cchar` is empty — on every line **except** the active window's cursor line, which stays fully revealed (Vim's default `concealcursor=""`). Rules are compiled once at startup into `Editor::conceal_compiled`; per-row ranges come from `conceal_line_ranges` and are stored in `RowSignature.conceal`, so the row cache repaints correctly as the cursor moves on/off a line and as `:set conceal` toggles. Concealment is applied in the styled glyph loop (hidden glyphs take zero columns; a cchar replaces the match's first glyph and the rest are dropped), so syntax coloring on the rest of the line is preserved.
- **Tests:** 2 Rust unit (`conceal_line_ranges` hides `**` at the right columns and replaces `->` with a cchar; `:set conceal`/`:set noconceal` toggle) + tests/pty_conceal.py (2 geometries: `**bold**`/`**more**` — the cursor line shows the markers, the other hides them; moving the cursor swaps which line is revealed).
- **Verified:** 565 Rust tests pass; clippy clean; PTY green.

### 2026-09-23 — Python block indent + dedent (2.10 follow-up)
- **Shipped:** `auto_indent` now (in a `.py`/`.pyi` buffer) adds one level after a line ending in `:` — `def`/`if`/`for`/`while`/`class`/`with`/`try`/`else`/… — which the bracket heuristic missed since Python blocks aren't `{}`-delimited; and dedents one level after a suite-ending statement (`return`/`pass`/`raise`/`break`/`continue`, matched whole-word so `returns_x` is exempt) via a `dedent_one` helper. Both gated on the file extension (`buf_is_python`) so other languages are unaffected. (Comment-after-colon handling remains part of the larger full-tree-sitter-indent follow-up.)
- **Tests:** 2 Rust unit (colon indent + language gating; suite-ender dedent incl. nested-one-level and the `returns_x` whole-word guard) + tests/pty_python_indent.py (2 geometries: `if x:`⏎`pass`⏎`y = 1` yields `if x:\n    pass\ny = 1` on disk — indent then dedent).
- **Verified:** 563 Rust tests pass; clippy clean; PTY green.

### 2026-09-23 — project-wide replace preview (2.7 follow-up)
- **Shipped:** `:cfarpreview /pat/repl/[flags]` (alias `:farpreview`) shows every line the matching `:cfar` would rewrite across the results/quickfix files in a Results panel, **without touching disk**. Reuses the exact `:s`/`:cfar` semantics via a shared `far_regex` helper (translated pattern + replacement, `g`/`i`/`I` flags, ignorecase/smartcase), reads open buffers in their live state and the rest from disk, and is bounded at 5000 entries. Run `:cfar` to apply. Per-hunk accept/reject remains.
- **Tests:** 1 Rust unit — three results files, `:cfarpreview /old/new/g` lists the 3 changed lines (`new value`, `keep new too`, `another new here`) and asserts all three files are byte-for-byte unchanged on disk (matching how the existing `:cfar` tests are structured, editor-level).
- **Verified:** 561 Rust tests pass; clippy clean.
...
- **Tests:** tests/pty_semantic_delta.py (2 geometries, mock LSP advertising `full.delta`): the initial full request colors line 0; an edit triggers a `full/delta` whose appended token turns line 3 cyan; the test also asserts a `semanticTokens/full/delta` request was actually logged. Verified the base semantic-tokens PTY (now with a `resultId`) still passes.
- **Verified:** 560 Rust tests pass; clippy clean; PTY green.

### 2026-09-23 — session fold-state persistence (6.x follow-up)
- **Shipped:** `:sessionsave` now also records each open buffer's fold ranges (`(start, end, closed)`, keyed by path) in `.vaayu/session.json`; `:sessionload` restores them onto the reopened buffers, clamped to the current line count and dropping degenerate/out-of-range ranges (bounded at 10k) so a stale or edited-since session can't produce bad folds. Added as a `#[serde(default)]` `folds` field, so older session files (which have none) still deserialize unchanged.
- **Tests:** 1 Rust unit — create a closed fold, `:sessionsave`, then a *fresh* editor `:sessionload`s and finds the fold restored `(3, 8, closed)` (a real save→disk→load round-trip, matching how the other session tests are structured).
- **Verified:** 560 Rust tests pass; clippy clean (fold-state type factored into a `FoldState` alias).
