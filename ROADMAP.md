...
- [ ] conceal support — **M**
- [~] 2.11 Configurable statusline: `statusline` config format string (`%f`/`%F`/`%l`/`%c`/`%L`/`%m`/`%y`/`%p`/`%M`/`%%`), ruler stays on the right. **Winbar done** (`:set winbar`: per-pane top row with the relative path + tree-sitter enclosing-symbol breadcrumb, content shifts down, mouse-mapping aware). **Global statusline done** (`:set globalstatusline`/`laststatus3`: one shared status for the active window, per-pane statuses reclaimed as content). statuscolumn + multi-level breadcrumb = remaining — **M**
